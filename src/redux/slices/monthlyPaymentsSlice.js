...
import monthlyPaymentsData from '../../data/monthlyPayments.json'
import { resolveTenantStatus, getCurrentMonthYear } from '../../utils/monthlyPaymentHelpers'
...
const monthlyPaymentsSlice = createSlice({
  ...
  initialState: {
    tenants: monthlyPaymentsData.map(syncTenantStatus),
  },
  reducers: {
    ...
    addTenant: (state, action) => {
      ...
      state.tenants.unshift(syncTenantStatus({
        ...data,
    ...
    /** API: DELETE /monthly-payments/:id */
...
export const { markTenantPaid, addTenant, updateTenant, refreshTenantStatuses, deleteTenant } = monthlyPaymentsSlice.actions
